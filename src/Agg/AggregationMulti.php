...
class AggregationMulti implements AggregationInterface
{
    ...
    public function generateResults($results)
    {
        ...
        foreach ($this->aggs as $label => $agg) {
            $prefix = $i++ . "_";
            $out['options'][$label] = $agg->generateResults($resultSets[$prefix]);
            if (is_numeric($out['options'][$label])) {
                $out['Total'] += $out['options'][$label];
            } elseif (isset($out['options'][$label]['Total'])) {
                $out['Total'] += $out['options'][$label]['Total'];
            }
        }
